"""Re-score the already-collected batch25 raw outputs (no new LLM calls)
with the keyword-injection check included, and split results by evidence
source (GitHub/project_inventory vs real work history/experience_
inventory) to see whether the fabrication problem concentrates on the
thin-evidence project entries specifically.
"""
...
def main() -> None:
    ...
    overall = {"github": {"old": [0, 0, 0], "new": [0, 0, 0]}, "non_github": {"old": [0, 0, 0], "new": [0, 0, 0]}}
    # [offered, survived_old_checks, survived_old_checks_and_no_injection]
    detail_examples = []

    for tag, model in (("qwen3", "qwen3:1.7b"), ("apex", "Apex"), ("qwen25", "qwen2.5:3b")):
        d = json.load(open(OUT_DIR / f"batch25_results_{tag}.json", encoding="utf-8"))
        model_stats = {"github": {"old": [0, 0], "new": [0, 0]}, "non_github": {"old": [0, 0], "new": [0, 0]}}
        for entry in d:
            job_id = entry["job_id"]
            row = conn.execute(
                "SELECT rowid, title, full_description FROM jobs WHERE rowid=?", (job_id,)
            ).fetchone()
            if row is None:
                continue
            job = dict(row)
            # Exclude jobs the real seniority gate would block before ever
            # reaching local realization -- matches production behavior.
            ...
            for label in ("old", "new"):
                ...
                for b in raw_bullets or []:
                    ...
                    model_stats[category][label][0] += 1  # offered
                    if passed_narrow and not kw["keyword_injected"]:
                        model_stats[category][label][1] += 1  # survived, corrected
                        ...
                        detail_examples.append(
                            {
                                ...
                                "evidence": name,
                                "text": text,
                                "injected_terms": kw["injected_terms"],
                            }
                        )

        print(f"\n=== {model} (corrected: narrow checks + keyword-injection, seniority-gated jobs excluded) ===")
        for category in ("github", "non_github"):
            for label in ("old", "new"):
                ...
                oo, os_ = overall[category][label][:2]
                overall[category][label][0] += offered
                overall[category][label][1] += survived

    print("\n\n=== TOTALS ACROSS ALL 3 MODELS ===")
    for category in ("github", "non_github"):
        for label in ("old", "new"):
            offered, survived = overall[category][label][:2]
            rate = survived / offered if offered else 0.0
            print(f"  {category:10s} / {label}: {survived}/{offered} = {rate:.1%}")

    print("\n=== Examples caught ONLY by the keyword-injection check (passed the narrow checks, still bad) ===")
    ...
    (OUT_DIR / "rescore_summary.json").write_text(
