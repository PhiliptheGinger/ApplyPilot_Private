...
def render_docx(resume: dict, output_path: str, metadata: dict | None = None) -> None:
    ...
    _finalize_docx(doc, output_path, metadata)


def _finalize_docx(doc, output_path: str, metadata: dict | None = None) -> None:
    """Populate core_properties, save, and scrub python-docx fingerprints.

    Shared tail for every DOCX writer in this module (``render_docx`` and
    ``render_plain_text_docx``) so metadata handling and fingerprint
    scrubbing can never drift between them.

    Always overwrites python-docx's template defaults (author='python-docx',
    comments='generated by python-docx', created/modified=2013-12-23) so
    reviewers + ATS systems don't see tooling fingerprints.
    """
...
def _split_into_paragraphs(text: str) -> list[str]:
    """Split freeform text into paragraphs on blank-line boundaries,
    preserving each paragraph's content verbatim and in order. A paragraph
    that happens to span multiple physical lines (not the normal cover-
    letter shape, where each paragraph is already one line) has its lines
    joined with a single space rather than losing the extra lines."""
    paragraphs: list[str] = []
...
def render_plain_text_docx(text: str, output_path: str, metadata: dict | None = None) -> None:
    """Render freeform prose (e.g. a cover letter) to DOCX as one paragraph
    per blank-line-separated source paragraph, verbatim and in order.

    2026-08-28 fix: cover letters were previously run through
    ``parse_resume``/``render_docx`` -- a resume-section parser/renderer
    with no concept of freeform prose. Since a cover letter never contains
    a literal "SUMMARY" header line, ``parse_resume``'s header-scanning
    loop never breaks, so it swallowed every line after the first blank
    line into a single ``title`` string (destroying every paragraph break),
    and its section-splitting loop never found an ALL-CAPS header either,
    so the rest of the letter's structure was silently dropped entirely.
    This function is a deliberately separate, minimal path for prose
    content so it never goes anywhere near resume-shaped parsing.
    """
    from docx import Document
...
def build_plain_text_html(text: str) -> str:
    """Render freeform prose (e.g. a cover letter) as simple paragraph-per-
    line HTML for the PDF path -- the PDF-path counterpart to
    ``render_plain_text_docx``, deliberately not routed through
    ``build_html`` (which is resume-section-shaped, same category-error
    risk as ``parse_resume``/``render_docx`` for prose content)."""
    from html import escape as _esc

    body = "\n".join(f"<p>{_esc(p)}</p>" for p in _split_into_paragraphs(text))
    return (
...
def convert_to_pdf(
    ...
    Args:
        ...
        content_type: "resume" (default) parses the input with
            ``parse_resume`` and renders it with the resume-shaped
            renderers. "cover_letter" skips resume parsing entirely and
            renders the text verbatim as one paragraph per source
            paragraph -- see ``render_plain_text_docx``/
            ``build_plain_text_html`` for why cover letters must never go
            through the resume-section parser (2026-08-28 fix).
    ...
    if content_type not in ("resume", "cover_letter"):
        raise ValueError(f"Invalid content_type '{content_type}'. Must be 'resume' or 'cover_letter'.")
    ...
    if content_type == "cover_letter":
        if html_only:
            html = build_plain_text_html(text)
        ...
        if doc_format == "docx":
            ...
            out = Path(out)
            render_plain_text_docx(text, str(out), metadata=metadata)
