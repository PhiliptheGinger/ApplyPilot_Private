...
def _extract_domain(address: str) -> str:
    ...
    try:
        ...
        return host.lower()
    except Exception:  # noqa: BLE001 - malformed address/URL degrades to a best-effort lowercase fallback, not a crash
...
def match_email_to_job(email: dict, applied_jobs: list[dict]) -> dict | None:
    ...
    for job in applied_jobs:
        ...
        if (
            url_company
            and len(url_company) > 2
            and not _sig2_fired
            and (url_company in subject or _slug(url_company) in _slug(subject))
        ):
            score += 25
        ...
        if subject_company_norm and company and len(subject_company_norm) > 2:
            ...
            if job_company_norm and (
                ...
                or (
                    subj_slug and job_slug and (subj_slug == job_slug or subj_slug in job_slug or job_slug in subj_slug)
                )
            ):
                score += 35
