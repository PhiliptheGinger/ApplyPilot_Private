...
def test_stealth_overrides_apply_in_main_world(cft_available):
    ...
    with tempfile.TemporaryDirectory() as td, sync_playwright() as p:
        ...
        try:
            ...
            assert results["stealthMarker"] is True, f"stealth.js not loaded in MAIN world; got {results}"
            assert results["webdriverUndefined"] is True
            assert results["pluginsLength"] >= 1
            assert results["pluginsIsArray"] is True, "navigator.plugins must pass `instanceof PluginArray`"
            assert results["firstIsPlugin"] is True, "individual plugin entries must pass `instanceof Plugin`"
