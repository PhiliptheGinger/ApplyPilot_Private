...
def test_databricks_gh_jid_to_canonical():
    ...
    assert canonicalize_application_url(src) == "https://job-boards.greenhouse.io/databricks/jobs/6779084002"


def test_stripe_gh_jid_to_canonical():
    src = "https://stripe.com/jobs/search?gh_jid=7393169"
    assert canonicalize_application_url(src) == "https://job-boards.greenhouse.io/stripe/jobs/7393169"


def test_pinterest_careers_to_canonical():
    src = "https://www.pinterestcareers.com/jobs/?gh_jid=7685119"
    assert canonicalize_application_url(src) == "https://job-boards.greenhouse.io/pinterest/jobs/7685119"


def test_airbnb_subdomain_host_match():
    src = "https://careers.airbnb.com/positions/7744247?gh_jid=7744247"
    assert canonicalize_application_url(src) == "https://job-boards.greenhouse.io/airbnb/jobs/7744247"


def test_extra_query_params_dropped():
    src = "https://cast.ai/careers/apply/?gh_jid=4136062009&gh_src=5rago2ff9us&urlHash=NeaV"
    assert canonicalize_application_url(src) == "https://job-boards.greenhouse.io/castai/jobs/4136062009"
...
def test_slug_map_has_top_employers():
    ...
    for host in ("databricks.com", "stripe.com", "pinterestcareers.com", "careers.airbnb.com"):
...
def test_parse_slug_from_canonical_iframe_src():
    ...
    assert (
        parse_greenhouse_slug_from_iframe_src("https://job-boards.greenhouse.io/databricks/jobs/6779084002")
        == "databricks"
    )
...
def test_parse_slug_from_legacy_boards_host():
    ...
    assert (
        parse_greenhouse_slug_from_iframe_src("https://boards.greenhouse.io/andurilindustries/jobs/4754841007")
        == "andurilindustries"
    )
...
def test_parse_slug_unknown_host_returns_none():
    ...
    assert parse_greenhouse_slug_from_iframe_src("https://example.com/jobs/123") is None
...
def test_register_runtime_slug_adds_unknown_host(isolated_runtime_cache):
    ...
    assert (
        canonicalize_application_url("https://acme-newco.io/careers?gh_jid=999")
