...
def test_pending_score_excludes_stale(tmp_db, seed_job):
    ...
    conn = tmp_db()
    seed_job(conn, url_suffix="fresh", fit_score=None, full_description="x", discovered_at=_iso(1), state="enriched")
    seed_job(conn, url_suffix="stale", fit_score=None, full_description="x", discovered_at=_iso(30), state="enriched")
