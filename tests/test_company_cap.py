...
def test_in_flight_counts_applied_in_progress_needs_human(tmp_db, seed_job):
    ...
    conn = tmp_db()
    seed_job(conn, url_suffix="a1", company="acme", apply_status="applied", applied_at=_iso(5))
    seed_job(
        conn, url_suffix="a2", company="acme", apply_status="in_progress", applied_at=None, last_attempted_at=_iso(1)
    )
    seed_job(
        conn, url_suffix="a3", company="acme", apply_status="needs_human", applied_at=None, last_attempted_at=_iso(2)
    )
    seed_job(conn, url_suffix="a4", company="acme", apply_status="failed", applied_at=_iso(3))
    seed_job(conn, url_suffix="a5", company="acme", apply_status="manual", applied_at=_iso(4))
...
def test_in_flight_uses_applied_at_or_last_attempted(tmp_db, seed_job):
    ...
    seed_job(
        conn,
        url_suffix="no-applied-at",
        company="acme",
        apply_status="in_progress",
        applied_at=None,
        last_attempted_at=_iso(7),
    )
    result = get_in_flight_by_company(conn)
...
def test_in_flight_case_insensitive_company(tmp_db, seed_job):
    ...
    conn = tmp_db()
    seed_job(conn, url_suffix="x1", company="Netflix", apply_status="applied", applied_at=_iso(1))
    seed_job(conn, url_suffix="x2", company="NETFLIX", apply_status="applied", applied_at=_iso(1))
    result = get_in_flight_by_company(conn)
...
def test_in_flight_uses_same_key_as_acquire_job(tmp_db, seed_job):
    ...
    job_kwargs = {
        "company": None,
        "site": "Temporal Technologies",
        "strategy": "greenhouse_api",
        "application_url": "https://job-boards.greenhouse.io/temporaltechnologies/jobs/123",
    }
    seed_job(conn, url_suffix="t1", apply_status="applied", applied_at=_iso(1), **job_kwargs)
    ...
    assert acquire_key in in_flight, f"acquire-time key {acquire_key!r} not in in_flight buckets {sorted(in_flight)!r}"
...
def test_acquire_job_respects_per_company_override(tmp_db, seed_job, monkeypatch, tmp_path):
    ...
    conn = tmp_db()
    for i in range(2):
        seed_job(conn, url_suffix=f"nf-{i}", company="netflix", apply_status="applied", applied_at=_iso(1))
    seed_job(
        conn,
        url_suffix="nf-new",
        company="netflix",
