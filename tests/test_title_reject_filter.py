...
from applypilot.database import get_jobs_by_stage, init_db, reject_jobs_by_title_patterns
...
def test_reject_titles_does_not_archive_tailored_jobs(tmp_path: Path):
    ...
    _seed_job(
        conn,
        url="https://example.com/tailored-senior",
    ...
    assert result["updated"] == 0
    row = conn.execute(
        "SELECT state, apply_category, apply_error FROM jobs WHERE url = ?", ("https://example.com/tailored-senior",)
    ).fetchone()
...
def test_reject_titles_does_not_archive_cover_failed_or_ready_to_apply(tmp_path: Path):
    ...
    _seed_job(
        conn,
        url="https://example.com/cover-failed-senior",
        title="Senior Platform Engineer",
        state="cover_failed",
        fit_score=9,
        tailored_resume_path="/tmp/resume.txt",
    ...
    _seed_job(
        conn,
        url="https://example.com/ready-senior",
        title="Senior Platform Engineer",
    ...
    cover_failed = conn.execute(
        "SELECT state FROM jobs WHERE url = ?", ("https://example.com/cover-failed-senior",)
    ).fetchone()
...
def test_pending_cover_only_selects_valid_states(tmp_path: Path):
    ...
    _seed_job(
        conn,
        url="https://example.com/pending-tailored",
    ...
    _seed_job(
        conn,
        url="https://example.com/pending-cover-failed",
        title="Software Engineer",
        state="cover_failed",
        fit_score=9,
        tailored_resume_path="/tmp/resume.txt",
    ...
    _seed_job(
        conn,
        url="https://example.com/archive-should-skip",
        title="Software Engineer",
        state="archived",
        fit_score=9,
        tailored_resume_path="/tmp/resume.txt",
