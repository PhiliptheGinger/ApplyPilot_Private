...
def test_save_then_load_roundtrip(tmp_paths_dir):
    from applypilot.apply.successful_paths import load_path, save_path
...
def test_save_caps_to_max_steps(tmp_paths_dir):
    from applypilot.apply.successful_paths import MAX_STEPS, load_path, save_path
...
def test_format_for_prompt_renders_section(tmp_paths_dir):
    from applypilot.apply.successful_paths import format_path_for_prompt, load_path, save_path
...
def test_keep_fastest_when_durations_present(tmp_paths_dir):
    ...
    save_path("greenhouse", [{"tool": "fast", "summary": "fast path step"}], duration_ms=240_000)
    # Slower run — should be skipped
    result = save_path("greenhouse", [{"tool": "slow", "summary": "slow path step"}], duration_ms=857_000)
...
def test_faster_run_wins_over_existing(tmp_paths_dir):
    ...
    save_path("greenhouse", [{"tool": "slow", "summary": "first slow run"}], duration_ms=857_000)
    result = save_path("greenhouse", [{"tool": "fast", "summary": "second fast run"}], duration_ms=240_000)
...
def test_first_save_always_writes(tmp_paths_dir):
    ...
    out = save_path("ashby", [{"tool": "x", "summary": "first time on ashby"}], duration_ms=999_000)
...
def test_legacy_file_without_duration_gets_replaced(tmp_paths_dir):
    ...
    _write_legacy_memo(tmp_paths_dir, "icims", [{"tool": "old", "summary": "legacy run"}])
    # New timed run — even a very slow one — must win over legacy.
    result = save_path("icims", [{"tool": "new", "summary": "first timed run"}], duration_ms=999_000)
    assert result is not None
...
def test_load_legacy_format(tmp_paths_dir):
    ...
    _write_legacy_memo(tmp_paths_dir, "workday", [{"tool": "browser_click", "summary": "browser_click Apply"}])
    loaded = load_path("workday")
    assert loaded is not None
...
def test_load_skips_stale_memo(tmp_paths_dir):
    ...
    save_path("greenhouse", [{"tool": "x", "summary": "step"}], duration_ms=240_000)
    out_path = PATHS_DIR / "greenhouse.json"
    payload = _json.loads(out_path.read_text())
    payload["captured_at"] = (datetime.now(UTC) - timedelta(days=MAX_AGE_DAYS + 5)).isoformat()
...
def test_stale_memo_overwritten_by_new_save(tmp_paths_dir):
    ...
    save_path("greenhouse", [{"tool": "fast", "summary": "fast ancient"}], duration_ms=100_000)
    out_path = PATHS_DIR / "greenhouse.json"
    payload = _json.loads(out_path.read_text())
    payload["captured_at"] = (datetime.now(UTC) - timedelta(days=MAX_AGE_DAYS + 20)).isoformat()
    out_path.write_text(_json.dumps(payload))

    # A newer SLOW save should still win because the ancient one is stale
    result = save_path("greenhouse", [{"tool": "slow", "summary": "slow but recent"}], duration_ms=900_000)
    assert result is not None
    loaded = load_path("greenhouse")
